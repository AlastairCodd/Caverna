...
class PointCalculationService(object):
    ...
    def __init__(self):
        self._point_actions: List[Callable[[Player], PointLookup]] = [
            # +1 per animal
    ...
    def _calculate_conditional_points(self, player: Player) -> List[PointLookup]:
    ...
    def _point_per_resource(self, resource_type: ResourceTypeEnum, player: Player) -> PointLookup:
        positive_points = player.get_resources_of_type(resource_type)
        result = PointLookup(positive_points)
        return result

    def _negative_per_missing_animal(self, animal_type: ResourceTypeEnum, player: Player) -> PointLookup:
