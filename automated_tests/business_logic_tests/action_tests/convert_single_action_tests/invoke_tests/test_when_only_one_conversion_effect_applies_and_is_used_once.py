...
class test_when_only_one_conversion_effect_applies_and_is_used_once(Given_A_ConvertSingleAction):
    ...
    def initialise_player(self) -> BasePlayerRepository:
        ...
        conversion_effect: ConvertEffect = ConvertEffect(
            {ResourceTypeEnum.wood: 2},
            {ResourceTypeEnum.coin: 4}
        )
        conversion_tile: BaseTile = MockTile(effects=[conversion_effect])
    ...
    def test_then_result_value_should_be_expected(self) -> None:
        self.assertEqual(self._result.value, 4)
